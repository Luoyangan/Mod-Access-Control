...
public final class MacCommand {
    ...
    private static SuggestionProvider<CommandSourceStack> suggestPolicyIds(boolean whitelist) {
        return suggestIds(() -> {
            List<String> ids = new ArrayList<>();
            for (PolicyEntry e : policy(cfg(), whitelist)) {
                ids.add(e.getId());
            }
            return ids;
        });
    ...
    private static String sourceLanguage(CommandSourceStack src) {
        // 1.16.5 的 CommandSourceStack 没有 getPlayer()，用 getEntity() 判断执行者是否为玩家。
        // Fabric 原版无语言接口，改用客户端在握手阶段上报的语言（服务端会话中保存）。
        return src.getEntity() instanceof ServerPlayer
                ? Holder.service().clientLanguage(((ServerPlayer) src.getEntity()).getStringUUID())
                : null;
    }
    ...
    private static int check(CommandContext<CommandSourceStack> ctx, String name) {
        ...
        if (s == null) {
            ctx.getSource().sendFailure(new TextComponent(CommandText.checkNotFound(name)));
            return 0;
        }
        send(ctx, s);
        return 1;
    ...
    private static void send(CommandContext<CommandSourceStack> ctx, String text) {
        MutableComponent line = new TextComponent(text).withStyle(ChatFormatting.GREEN);
