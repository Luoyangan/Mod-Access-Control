...
public final class MacCommand {
    ...
    private static int run(CommandContext<CommandSource> ctx, Op op) {
        try {
            ...
            ctx.getSource().sendFailure(new StringTextComponent("[MAC] 命令执行失败: " + ex.getMessage()));
            return 0;
        }
    }
    ...
    private static int check(CommandContext<CommandSource> ctx, String name) {
        ...
        if (s == null) {
            ctx.getSource().sendFailure(new StringTextComponent("[MAC] 找不到玩家: " + name));
            return 0;
        }
        send(ctx, s);
        return 1;
    ...
    private static int addId(CommandContext<CommandSource> ctx, String name, List<String> cur,
    ...
    private static int removeId(CommandContext<CommandSource> ctx, String name, List<String> cur,
                                String id, Consumer<MacConfig> apply) {
        boolean had = false;
        for (String i : cur) {
            if (i.equalsIgnoreCase(id)) {
