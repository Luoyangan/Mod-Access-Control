...
/**
 * Forge 网络适配层（MC 1.16.5 / Forge 36.x）。
 *
 ...
 *
 * <p>1.16.5 与 1.20.1 同属旧版 SimpleChannel 体系，但位于 {@code fml.network} 包；
 * JSON 结构仍由 common 核心定义。</p>
 */
public final class ForgeNet {
    ...
    public static void init() {
        ...
        channel.messageBuilder(Stage1RequestS2C.class, id++, NetworkDirection.PLAY_TO_CLIENT)
                ...
                .consumer(Stage1RequestS2C::handle)
                .add();
        // C2S：登录阶段应答
        channel.messageBuilder(Stage1ResponseC2S.class, id++, NetworkDirection.PLAY_TO_SERVER)
                ...
                .consumer(Stage1ResponseC2S::handle)
                .add();
        // S2C：进入游戏阶段请求（完整列表）
        channel.messageBuilder(Stage2RequestS2C.class, id++, NetworkDirection.PLAY_TO_CLIENT)
                ...
                .consumer(Stage2RequestS2C::handle)
                .add();
        // C2S：进入游戏阶段应答
        channel.messageBuilder(Stage2ResponseC2S.class, id++, NetworkDirection.PLAY_TO_SERVER)
                ...
                .consumer(Stage2ResponseC2S::handle)
                .add();
        Mac.logger().info("Forge 网络通道已注册 (channel={}:{})", Mac.MOD_ID, Mac.CHANNEL_PATH);
    }

    /** 服务端向单个玩家发送一条服务端请求。 */
    public static void sendToPlayer(ServerPlayerEntity player, String kind, String payloadJson) {
    ...
    public static boolean remoteHasChannel(NetworkManager connection) {
    ...
    private static void respondStage1(String json) {
        ...
        resp.modVersions = new HashMap<String, String>();
        List<String> ids = req.requiredIds == null ? Collections.<String>emptyList() : req.requiredIds;
        for (String rid : ids) {
    ...
    private static void respondStage2() {
        ...
        resp.mods = new ArrayList<ClientMod>();
    ...
    public static final class Stage1RequestS2C {
        ...
        static void encode(Stage1RequestS2C m, PacketBuffer buf) {
            buf.writeUtf(m.json);
        }

        static Stage1RequestS2C decode(PacketBuffer buf) {
    ...
    public static final class Stage1ResponseC2S {
        ...
        static void encode(Stage1ResponseC2S m, PacketBuffer buf) {
            buf.writeUtf(m.json);
        }

        static Stage1ResponseC2S decode(PacketBuffer buf) {
        ...
        static void handle(Stage1ResponseC2S m, Supplier<NetworkEvent.Context> sup) {
            ...
            try {
                final Stage1Response resp = Json.fromJson(m.json, Stage1Response.class);
                if (resp != null) {
                    ctx.enqueueWork(() -> Holder.service().receiveStage1(sp.getStringUUID(), resp));
                }
            } catch (Exception e) {
    ...
    public static final class Stage2RequestS2C {
        ...
        static void encode(Stage2RequestS2C m, PacketBuffer buf) {
            buf.writeUtf(m.json);
        }

        static Stage2RequestS2C decode(PacketBuffer buf) {
    ...
    public static final class Stage2ResponseC2S {
        ...
        static void encode(Stage2ResponseC2S m, PacketBuffer buf) {
            buf.writeUtf(m.json);
        }

        static Stage2ResponseC2S decode(PacketBuffer buf) {
        ...
        static void handle(Stage2ResponseC2S m, Supplier<NetworkEvent.Context> sup) {
            ...
            try {
                final Stage2Response resp = Json.fromJson(m.json, Stage2Response.class);
                if (resp != null) {
                    ctx.enqueueWork(() -> Holder.service().receiveStage2(sp.getStringUUID(), resp));
                }
            } catch (Exception e) {
